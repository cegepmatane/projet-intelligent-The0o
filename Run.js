class Run {
    ...
    async run() {
        while (!this.PERDU) {
            let coordonnees = document.getElementById("contenant").getBoundingClientRect();
            let xRandom = Math.random() * ((coordonnees.x + coordonnees.width) - coordonnees.x) + coordonnees.x;
            this.ajouterFruit(xRandom, 200);
            await this.sleep(200);
        }
        console.log("Score " + this.numeroFruit);
        this.listeResultat.push(this.numeroFruit / 200)
        this.PERDU = false;
    }
    ...
    async runPopulationInitiale() {
        ...
        console.log(this.listeResultat);
    }
